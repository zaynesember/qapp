"""
Module for state_po cleaning tools.
"""
...
class State_po(field.Field):
    DEFAULT_TEXT_CHECKS = {
        # Only allow A-Z
        'UNRECOGNIZED CHARACTERS': r'[^A-Z]+',
    ...
    def check_special(self, data: DataFrame,
        ...
        if verbose:
            print('*Starting correct state po for state check...', flush=True)
        fileio.make_dir_if_needed(filename)
        po_values = miscellaneous.obtain(column=data['state_po'], intended_type=str)
        ...
        def _state_code():
            ...
            if len(po_values) > 1:
                issues = True
                return (issues,\
                        'Unable to report on correct state po for state check: multiple '
                        'state_po values found.')
            if 'state' not in data:
                issues = True
                return (issues,\
                        'Unable to report on correct state po for state check attributes: state '
                        'field missing.')
            if not pathlib.Path(state_codes_location).exists():
                issues = True
                return (issues,\
                        f'Unable to report on correct state po for state check attributes: state '
                        f'code file {state_codes_location} missing.')
            state_values = miscellaneous.obtain(column=data['state'], intended_type=str)
            if len(po_values) > 1:
                issues = True
                return (issues,\
                        'Unable to report on correct state po for state check: multiple state '
                        'values found.')

            po_value = po_values[0]
            state_value = state_values[0]
            try:
                expected_po = state.State.get_state_code(state_codes_location, state_value,
                                                         'state_po')
            except KeyError as exc:
                issues = True
                return (issues,\
                        f'Unable to report on correct state po for state check: {exc}.')

            return (issues, \
                    f'*Expected state po code {expected_po} for {state_value}, '
                    f'found {po_value}.')
        ...
        output.append('------\n')
        output.append('STATE PO FOR STATE CHECK:\n')
